...
// Import listS3Objects from the src/services version
export { listS3Objects } from './src/services/s3Service';
        ...
        try {
          ...
          for (const { newKey } of successfulCopies) {
            try {
              ...
              console.error('Error undoing copy operation');
            }
          }
...
const copyS3Object = async (sourceKey, destinationKey) => {
  try {
    ...
    console.error('Error copying object');
...
const getHistoryLog = async () => {
  try {
    ...
    console.error('Error fetching history log');
    return [];
  }
};
...
const getBucketMetrics = async () => {
  try {
    // Import listS3Objects directly from the module where it's defined
    const { listS3Objects } = await import('./src/services/s3Service');
    ...
    console.error('Error getting bucket metrics');
    throw new Error('Failed to get bucket metrics');
  }
};
